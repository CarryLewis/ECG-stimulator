...
| `ventricular_flutter` | Arrhythmia | Rapid regular sine-wave ventricular reentry (~300/min) |
...
**v1 (this delivery):**  
Architecture, extensible interface, registry/engine, disease library of physiological modifiers, `PhysiologicalModel → CyclePlan` bridge, and shared dipole sampling that drives both the 3D heart glow and the live 12-lead ECG for the teaching pathology scenarios (AV block, AF, atrial flutter, ventricular flutter, VF, STEMI).

**Later (EP engine work):**  
Full Wenckebach counters, AF RR stochastic schedules, flutter F-wave vectors, VT exit-site axes, and VF wavelet synthesis can deepen inside the EP/vector engines — already parameterized by these packs.
