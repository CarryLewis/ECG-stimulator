...
const aflutter = resolveDiseaseSimulation({ diseaseId: 'atrial_flutter' })
assert(aflutter.model.atrialMode === 'flutter', 'atrial flutter mode')
assert((aflutter.model.flutterCycle_s ?? 0) > 0, 'flutter cycle set')

const vflutter = resolveDiseaseSimulation({
  diseaseId: 'ventricular_flutter',
  params: { flutterRate: 300, amplitude: 90 },
})
assert(vflutter.model.ventricularMode === 'flutter', 'ventricular flutter mode')
assert(vflutter.model.pAmplitudeScale === 0, 'VFlutter suppresses P')

const vf = resolveDiseaseSimulation({ diseaseId: 'ventricular_fibrillation' })
assert(vf.model.ventricularMode === 'fibrillation', 'VF mode')
assert(vf.model.pAmplitudeScale === 0, 'VF suppresses P')
  ...
  JSON.stringify(
    {
      ...
      sample: {
        ...
        ventricular_flutter: {
          mode: vflutter.model.ventricularMode,
          rate: vflutter.model.ventricularRate_bpm,
        },
      },
    },
    null,
